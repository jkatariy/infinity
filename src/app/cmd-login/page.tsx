...
export default function CMDLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Check if the logged-in user is the CMD user
      const { data: { user } } = await supabase.auth.getUser();
      if (user?.email !== 'cmd@infinitysols.com') {
        await supabase.auth.signOut();
        setError('Access denied. This login is for CMD personnel only.');
        setLoading(false);
        return;
      }

      router.push('/cmd-dashboard');
    } catch (error) {
        ...
        <div className="bg-white/80 backdrop-blur-xl p-8 rounded-2xl shadow-2xl border border-white/20 relative">
          ...
          <div className="text-center">
            ...
            <h2 className="text-3xl font-light text-gray-900 mb-2">
              CMD Dashboard Access
            </h2>
            <p className="text-sm text-gray-600 mb-8">
              Sign in to approve content and manage operations
            </p>
          </div>
