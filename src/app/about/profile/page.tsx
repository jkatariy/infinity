...
export default function Profile() {
  ...
  const timeline: { year: string; points: string[] }[] = [
    ...
    { year: '2025', points: ['Largest case packer for biscuit', 'Biggest project for staple foods in Africa', 'Biggest snack brand in Thailand'] },
  ];
  ...
  function AutoMarquee({ children }: { children: React.ReactNode }) {
    ...
    return (
      <div ref={containerRef} className="relative overflow-x-auto py-8 snap-x snap-mandatory" style={{ scrollbarWidth: 'none' }}>
        {children}
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-white pt-20 sm:pt-24 md:pt-24 lg:pt-24">
      {/* Top Banner (Engineering Boxed Theme) */}
      {/* Removed previous hero; using clean boxed banner to match clientele engineering theme */}
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {/* Evolution - Full Width Horizontal Timeline */}
            ...
            <AutoMarquee>
              <div className="flex items-stretch gap-6 w-max pr-4">
                {timeline.map((item, idx) => (
                  <div key={item.year} className="relative shrink-0 w-64 md:w-80 snap-start">
                    {/* Dot */}
                    <div
                      className="absolute left-1/2 top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-white shadow"
                      style={{ backgroundColor: idx === timeline.length - 1 ? '#2563EB' : '#06b6d4' }}
                    />
                    {/* Card */}
                    <div className="relative mt-8 rounded-xl border border-gray-200 bg-white p-5 shadow-sm hover:shadow-md transition-shadow">
                      <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-brand-blue-400 to-brand-blue-600" />
                      <div className="mt-2">
